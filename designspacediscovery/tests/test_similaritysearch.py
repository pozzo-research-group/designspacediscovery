...
def test_get_molecule_properties_validurlcall_parseresponse(
    ...
    assert mock_run_queries.call_args.args[0][0] == test_basis['1'], 'Wrong list passed to batch query'
    assert mock_run_queries.call_args.args[1] == correct_url_0, 'Wrong URL passed to batch query'

    assert list(results[0].keys())[0] == 'CID', 'Error in parsing of pubchem reponses'
...
def test_get_pubchem_vendor_status(mock_response, mock_run_queries):
    ...
    mock_json = {'SourceCategories': {'RecordType': 'CID',
                'RecordNumber': 445639,
                'Categories': [{'Category': 'Chemical Vendors',
                    'URL': 'https://www.ncbi.nlm.nih.gov/sites/entrez?cmd=search&db=pcsubstance&term=445639[StandardizedCID]+AND+%22Chemical+Vendors%22[SourceCategory]',
                    'Sources': [{'SID': 441561215,
                    'SourceName': 'Elsa Biotechnology',
                    'SourceURL': 'https://www.elsa-biotech.com',
                    'SourceDetail': 'https://pubchem.ncbi.nlm.nih.gov/source/Elsa%20Biotechnology',
                    'RegistryID': 'ELSA20-8625'},
                    {'SID': 355047057,
                    'SourceName': 'Yuhao Chemical',
                    'SourceURL': 'http://www.chemyuhao.com',
                    'SourceDetail': 'https://pubchem.ncbi.nlm.nih.gov/source/Yuhao%20Chemical',
                    'RegistryID': 'HL1416',
                    'SourceRecordURL': 'http://www.chemyuhao.com/112-80-1.html'}]}]}}
    mock_response.json.return_value = mock_json
    mock_response.status_code = 200
    mock_run_queries.return_value = {'1':mock_response}

    # caching logic is not checked here due to issue with picking mock objects
    results = sim.get_pubchem_vendor_status(test_basis, cache_params = {'cache':False})

    # test that retriever is called correctly
    assert list(mock_run_queries.call_args.args[0].values())[0] == correct_url_0, 'Incorrect url passed for pubchem vendor getting'
