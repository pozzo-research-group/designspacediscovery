...
def test_find_similar_molecules_validurlcall_parseresponse(mock_response, mock_run_queries):
    ...
    results = sim.find_similar_molecules(test_basis, max_records=1, threshold=99)

    assert list(mock_run_queries.call_args.args[0].values())[0] == correct_url_0, 'URL passed to pubchem query malformed'
...
@patch.object(designspacediscovery.querypubchem.pubchemQuery, 'run_queries')
@patch.object(requests.Response, 'json')
def test_get_molecule_properties_validurlcall_parseresponse(mock_response, mock_run_queries):
    # make sure a mock response is processed correctly
