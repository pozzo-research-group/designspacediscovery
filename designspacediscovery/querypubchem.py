...
class pubchemQuery():
    ...
    def run_queries(
        ...
        cache_count = 0
        cache_num = 0
        for key in tqdm(list(URLs.keys()), file=sys.stdout,
                        dynamic_ncols=True):
            URL = URLs[key]
            ...
            if cache:
                if cache_count > 1000:
                    ...
                    with open(f'{cache_fp}/{cache_name}_{cache_num}.pkl',
                              'wb') as f:
            ...
            if fail_count > 10:
                raise AssertionError(
                    'Check URLS or other inputs, 10 failed pubchem requests in a row. Stopping now'
                )
    ...
    def batch_queries(self, cid_list, url, max_batch_size=10000):
        ...
        for chunk in ut.chunked_iterable(cid_list, max_batch_size):
            #print(chunk)
            print(
                f'Batch query {100*chunk_num*max_batch_size/len(cid_list):.2f}% complete',
                end='\r')
            postbody = 'cid=' + ','.join(
                str(cid) for cid in chunk if ut.is_integery(cid))
            self.__check_rate_status__()
            fail_count = 0
            try:
    ...
    @backoff.on_exception(
        ...
        (requests.exceptions.HTTPError, requests.exceptions.ConnectionError,
         ...
         requests.exceptions.ReadTimeout),
        max_tries=5)
    def __execute_batch_query__(self, URL: str,
